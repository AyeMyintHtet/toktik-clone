        ...
        {topics.map((item: any) => (
          <Link href={`/?topic=${item.name}`} key={item.name}>
            <div className={topic === item.name ? activeTopicStyle : topicStyle}>
              <span className='font-bold text-2xl xl:text-md cursor-pointer'>{item.icon}</span>
              <span className='font-medium text-md hidden xl:block capitalize cursor-pointer'>{item.name}</span>
            </div>
          </Link>
        ))}
      </div>
